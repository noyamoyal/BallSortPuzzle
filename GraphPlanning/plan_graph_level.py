...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for action in all_actions:
            # If its preconditions are in the previous propositions layer and the preconditions are not pairwise mutex
    ...
    def update_mutex_proposition(self):
        ...
        for pro1 in current_layer_propositions:
            for pro2 in current_layer_propositions:
                if pro1 != pro2 and mutex_propositions(pro1, pro2, current_layer_mutex_actions):
                    self.proposition_layer.add_mutex_prop(pro1, pro2)
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    for pre1 in a1_pre_list:
        for pre2 in a2_pre_list:
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    for action1 in prop1_producers:
        for action2 in prop2_producers:

...
def independent_pair(a1, a2):
    ...
    a2_add_list = a2.get_add()
    a1_del_list = a1.get_delete()
    a2_del_list = a2.get_delete()
    # Check for inconsistent effect
    for pro in a1_add_list:
        if a2.is_neg_effect(pro):
            return False
    for pro in a2_add_list:
        if a1.is_neg_effect(pro):
            return False
    # Check for interference
    for pro in a1_del_list:
        if a2.is_pre_cond(pro):
            return False
    for pro in a2_del_list:
        if a1.is_pre_cond(pro):
            return False
    return True
